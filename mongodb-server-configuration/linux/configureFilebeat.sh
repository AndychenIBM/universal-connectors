...
filebeat_conf=$( readlink  -f /root/*/filebeat.yml )
...
sed -i -r "$startRange,$endRange{h;s/$ignoreCommentsString;s/enabled: false.*/enabled: true/g}" $filebeat_conf
if grep -qF "$path" $filebeat_conf;then
	printf "%s: path is already located in filebeat configuration. Please check %s\n" $(date +"%Y-%m-%dT%H:%M:%SZ") $filebeat_conf|& tee -a $logfile
else
    sed -i -r "$startRange,$endRange{h;s/$ignoreCommentsString;s/paths:*/paths:\n    - $path/g}" $filebeat_conf
fi
