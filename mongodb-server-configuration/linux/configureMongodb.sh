...
#Find mongod configuration file path to edit
mongod_conf_path=$( readlink  -f /*/mongod.conf )
...
then
    ...
    mongod_conf=$mongod_conf_path
    echo "created $mongod_conf.$current_time as a backup configuration file" |& tee -a $logfile
else
    #Edit example file
    cp $mongod_conf_path $mongod_conf_path.guardium_example.$current_time
    mongod_conf=$mongod_conf_path.guardium_example.$current_time
    echo "created $mongod_conf.guardium_example.$current_time as an example" |& tee -a $logfile
    echo $mongod_conf
fi
...
then
    ...
    printf "%s: Changed were made in example file. Mongod was not restarted.\n" $(date +"%Y-%m-%dT%H:%M:%SZ") |& tee -a $logfile
fi